...
{
    /*MSPrintf(EUSCI_A2_BASE, "AT+SYSIOSETCFG=16,3,1\r\n");
    __delay_cycles(12000);
    if(!ESP8266_WaitForAnswer(ESP8266_RECEIVE_TRIES))
    {
        printf("no answer set gpio mode\n");
        return false;
    ...
    if(strstr(ESP8266_Buffer, "OK") == NULL)
    {
        printf("cant set gpio mode\n");
        return false;
    }



    MSPrintf(EUSCI_A2_BASE, "AT+SYSGPIODIR=16,1\r\n");
    __delay_cycles(12000);
    if(!ESP8266_WaitForAnswer(ESP8266_RECEIVE_TRIES))
    {
        printf("no answer set output mode\n");
        return false;
    }

    if(strstr(ESP8266_Buffer, "OK") == NULL)
    {
        printf("cant set output mode: %s\n", ESP8266_Buffer);
        return false;
    }


    MSPrintf(EUSCI_A2_BASE, "AT+SYSGPIOWRITE=16,0\r\n");
    __delay_cycles(12000);
    if(!ESP8266_WaitForAnswer(ESP8266_RECEIVE_TRIES))
    {
        printf("no answer set low power mode\n");
        return false;
    }

    if(strstr(ESP8266_Buffer, "OK") == NULL)
    {
        printf("cant set low power mode\n");
        return false;
    }
